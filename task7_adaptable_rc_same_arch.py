...
#
# ── MODE (set at top) ─────────────────────────────────────────────────────────
#   "standard"   → standard RC, ρ=28 only, symmetric reservoir, 
#                  no squaring, no noise, no parameter injection
#                  → matches Zheng-Meng / reference code exactly
#
#   "adaptable"  → adaptable RC, ρ=25/28/31, symmetric reservoir,
#                  no squaring, no noise, parameter injection only
#                  → clean comparison: only bifurcation parameter differs
#
...
# import networkx as nx  # no longer needed (symmetric reservoir for both modes)
...
def build_batch_gpu(B, N, d, eig_rho, gamma, dim, seeds, mode="standard"):
    ...
    for s in seeds:
        if mode == "standard":
            ...
            rng_np  = np.random.default_rng(s)
            # Symmetric reservoir — identical architecture to standard RC
            # Only difference: Wp is active for bifurcation parameter injection
            S_dense = np.zeros((N, N), dtype=np.float32)
            mask    = rng_np.random((N, N)) < d
            weights = rng_np.standard_normal((N, N)).astype(np.float32)
            S_dense[mask] = weights[mask]
            ...
            if rho_a > 1e-12:
                A *= eig_rho / rho_a
            Win = (2 * rng_np.random((N, dim)).astype(np.float32) - 1) * gamma
            Wp  = rng_np.uniform(-1.0, 1.0, (N, 1)).astype(np.float32)
...
def streaming_train_checkpoints(rho, p_val, Win_b, A_b, Wp_b,
    ...
    while ws_done < washout:
        ...
        # No noise in either mode (clean comparison)
